...
class WorkshopC extends CI_Controller
{
    ...
    public function kembali($id_perizinan){
        define('EXT', '.'.pathinfo(__FILE__, PATHINFO_EXTENSION));
        // define('FCPATH', __FILE__);
        // define('SELF', pathinfo(__FILE__, PATHINFO_BASENAME));
        define('PUBPATH',str_replace(SELF,'',FCPATH));
        $berkas = $this->WorkshopM->get_berkas_by_id_perizinan($id_perizinan)->result();
        $jumlah_file = $this->WorkshopM->get_berkas_by_id_perizinan($id_perizinan)->num_rows();
        $i=0;
        foreach ($berkas as $ber) {
            if(unlink(PUBPATH."assets/upload/".$ber->nama_file)){
                $this->WorkshopM->hapus_berkas_perizinan($ber->nama_file);
                $i++;
            }else{
        ...
        // $this->WorkshopM->hapus_perizinan($id_perizinan);
        // redirect('izin_baru22');
        if($jumlah_file == $i){
            if($this->WorkshopM->hapus_perizinan($id_perizinan)){
                redirect('izin_baru22');
            }else{
                redirect_back();
            }
